...
class SharedExpenseCreationViewTests(TestCase):
    ...
    def test_create_personal_expense(self):
        ...
        expense = Expense.objects.filter(user=self.user, description='Test personal expense').first()
    ...
    def test_create_shared_expense_basic(self):
        ...
        # Verify expense was created
        expense = Expense.objects.filter(user=self.user, description='Shared dinner').first()
        self.assertIsNotNone(expense)
        self.assertEqual(expense.amount, Decimal('100.00'))
    ...
    def test_create_shared_expense_multiple_participants(self):
        ...
        # Verify expense was created
        expense = Expense.objects.filter(user=self.user, description='Group lunch').first()
        self.assertIsNotNone(expense)
    ...
    def test_participant_name_trimming(self):
        ...
        expense = Expense.objects.filter(user=self.user, description='Test trimming').first()
        shared_expense = expense.shared_details
...
class SharedExpenseListViewTests(TestCase):
    ...
    def test_shared_expense_indicator_displayed(self):
        ...
        self.assertEqual(response.status_code, 302, "Form submission should redirect on success")
        
        # Verify the shared expense was created
        shared_expenses = Expense.objects.filter(user=self.user, description='Shared dinner')
